...
import { LCDClient, MnemonicKey, TendermintSubscriptionResponse, Wallet, WebSocketClient } from '@terra-money/terra.js';
...
import { ACTIONABLE_ACTIONS, CHAIN_ID_LOCALTERRA, EVENT_ATTRIBUTE_KEY_ACTION, EVENT_TYPE_WASM, WEB_SOCKET_URL } from './constant';
...
export const getValueByKeyInAttributes = (attributes: TMEventAttribute[], k: string) => {
    ...
    if (val === '') {
        throw new Error(`please inspect manually, value not found by key: ${k} in attributes: ${attributes}`)
    }
    return val
}
