...
export const saveJob = async (
  ...
  if (reward === 0) {
    console.log(
      'reward too low, less than 0.001 luna, ignore the job, will add back if creator updates reward later'
    );
    return;
  }
...
export const findExecutableJobs = async (
  ...
  while (true) {
    ...
    for (let i = allJobIds.length - 1; i >= 0; i--) {
      ...
      const jobConditionStr = await redisClient.hGet('conditions', jobId!);
      const jobCondition = JSON.parse(jobConditionStr!);

      const jobVarsStr = await redisClient.hGet('vars', jobId!);
...
export const handleJobCreation = async (
  ...
  if (exist) {
    ...
    warpSdk
      ...
      .catch((err) => {
        console.log(
          'error getting job, probably due to rpc not updated to latest state',
          err
        );
        throw err;
      });
